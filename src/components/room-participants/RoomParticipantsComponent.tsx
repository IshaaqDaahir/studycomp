                ...
                {participants.map((participant: Room) => (
                    <Link key={participant.id} href={`/profile/${participant.id}/`} className="participant">
                        <div className="avatar avatar--medium">
                            <Image src={participant.avatar?.startsWith('https://') ? participant.avatar : `${process.env.NEXT_PUBLIC_DJANGO_API_URL}${participant.avatar}` || avatar} width={32} height={32} alt="Participant Avatar" />
                        </div>
                        <p>
